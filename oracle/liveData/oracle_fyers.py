...
class Fyers:
    ...
    def get_historicaldata(
        ...
        fyInstrument = f"{exchange}:{instrument}"
    ...
    def openConnection(self):
        ...
        symbols = Fyers.instruments
        self.fs.subscribe(symbols=symbols)
    ...
    def main(self, stopTime):
        ...
        self.fs = data_ws.FyersDataSocket(
            ...
            on_connect=self.openConnection,
            on_message=self.custom_message,
            on_error=Fyers.onerror,
            on_close=Fyers.onclose,
        )
        self.fs.connect()
    ...
    def get_livedata(
        ...
        if searchscrip:
            for ins in instruments:
                fyInstrument = InstrumentFinder(name=ins, brokerage="fyers")[1]
                Fyers.instruments.append(fyInstrument)

        else:
            Fyers.instruments = instruments

        Fyers.onmessage = onmessage
        Fyers.onerror = onerror
        Fyers.onclose = onclose
